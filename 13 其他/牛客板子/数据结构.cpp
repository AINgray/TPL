...
struct unionfind
{
	...
	unionfind(const int n) : f(n), siz(n, 1)
	{
		for (int i = 0; i < n; i++)
			f[i] = i;
	}
...
struct fenwick
{
	...
	{
		while (p < n)
			val[p] += v, p += lowbit(p + 1);
	}
	T query(int p) const
	{
		T re = 0;
		while (p >= 0)
			re += val[p], p -= lowbit(p + 1);
		return re;
	}
